...
{
	if ($rootScope.getglobaldata.Client === 'GearHead') {
		servicePath = $rootScope.getglobaldata.getGlobalServiceUrl + "CreateCases";
	}
	else{
...
function createNewWebTicket($scope,$rootScope,txtSummary,txtProblem)
{
	if ($rootScope.getglobaldata.Client === 'GearHead') {
             params = '{ "customerId":0, "registrationId": 0, "serialNumber": "' + $scope.selectedproductdet.SerialNo +
                        '", "caseSummary": "' + txtProblem + '","caseProblem": "' + txtProblem + '", "caseCauses": "", "caseNotes": "' + txtSummary +
                        '", "caseSource":"Online", "caseAssingTo":"", "caseQueueId":66,"caseType":"webticket"}'; //changed the QueueId 63 to 66
     }
     else{
