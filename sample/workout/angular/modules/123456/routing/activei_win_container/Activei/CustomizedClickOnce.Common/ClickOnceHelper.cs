        ...
        public static string GetUserUniqueID()
        {
            ...
            try
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    foreach (XmlNode node in doc.SelectNodes("/configuration/usersettings/add"))
                    {
                        if (node.Attributes["key"].InnerText == "NetgearToolUniqueID")
                        {
                            uniqueID = node.Attributes["value"].InnerText;
                            uniqueID = Cryption.Decrypt(uniqueID, Cryption.SecurityKey);
                            //errorTrace.WriteLog("GetUserUniqueID() :: uniqueID :: " + uniqueID);
                            break;
                        }
                    }
                }
            ...
            {
                //errorTrace.WriteErrorLog("App.xaml.cs", "GetUserUniqueID()", "", ex.Message, ex.StackTrace, "Error");
            }
            finally
            {
                //errorTrace = null;
