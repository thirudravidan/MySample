...
.controller('loginRightController', function loginRightController($scope, $rootScope, $http, $state, ngDialog) {
  ...
  $rootScope.userDetails.IsNetwork=((window.external.checknetwork()) ? 'True' : 'False');
  $("#networkstatus").removeClass('net_off network_init').addClass(($rootScope.userDetails.IsNetwork === "True") ? 'net_on' : 'net_off');
  ...
  $scope.rememberPass = function () {
      ...
      try {
          ...
          $rootScope.userLoginStatus = userDet;
          if (userDet[0] !== null) {
              if (($rootScope.userDetails.IsNetwork === 'False') && userDet[3] === 'false' && localStorage.getItem('StoredCustomerID') !=='' && localStorage.getItem('StoredCustomerID')!==null) {
                  $state.go('home');
              }
              else {
    ...
    $scope.userloginSer = function (usernameVal, passwordVal, serviceUrl) {
        try {
            if ($rootScope.userDetails.IsNetwork === 'True') {
                ...
                $state.go('home');
                // if ($rootScope.userLoginStatus[0] !== null) {
                //     $state.go('home');
                // }
                // else{
                //   var headerText = ($rootScope.getglobaldata.Client === 'GearHead') ? $rootScope.getglobalErrorMessage.APPHDRGEARHEAD : $rootScope.getglobalErrorMessage.APPLICATIONHDR;
...
function logDeviceStatus($rootScope, $scope, $http, $state) {
    ...
    var networkParam = { ipAddress: $rootScope.myinfo.MACAddress, networkMapDetails: JSON.stringify($rootScope.getConnectedDevices) };
