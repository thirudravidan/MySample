...
$scope.saveNewSchedule=function(isformvalid){
    ...
    if (validateDatetime($scope,$rootScope,ngDialog)) { //validate date and time if   
        ...
        // var patterns=['GMT ','UTC '];
        // var time='';
        // patterns.forEach(function(item){
        //    if($scope.scheduleTimeZone.Time.indexOf(item) > -1){
        //        time = $scope.scheduleTimeZone.Time.replace(item,'');
        //    }
        // });

        // alert(moment($("#txtTime").val(), "HH:mm zz").zone(time));
